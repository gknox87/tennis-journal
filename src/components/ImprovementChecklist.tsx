...
export const ImprovementChecklist = () => {
  ...
  const fetchImprovementPoints = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('improvement_points').select('*').order('created_at', {
        ascending: false
      }).limit(5);
  ...
  const togglePoint = async (id: string, currentValue: boolean) => {
    try {
      const {
        error
      } = await supabase.from('improvement_points').update({
  ...
  if (points.length === 0) {
    return <div className="text-center space-y-4">
        ...
        <Button variant="outline" size="sm" onClick={fetchImprovementPoints} disabled={isRefreshing} className="mx-auto">
  ...
  return <div className="space-y-4">
      <div className="flex items-center justify-between mb-4">
        <div className="space-y-2">
          {points.map(point => <div key={point.id} className="flex items-start space-x-3 text-left">
              <Checkbox id={point.id} checked={point.is_completed} onCheckedChange={() => togglePoint(point.id, point.is_completed)} className="mt-1 my-[3px] mx-[3px]" />
      ...
      <Button variant="outline" size="sm" onClick={fetchImprovementPoints} disabled={isRefreshing} className="w-full">
        <RefreshCw className={`h-4 w-4 mr-2 ${isRefreshing ? 'animate-spin' : ''}`} />
        Refresh Tips
      </Button>
    </div>;
