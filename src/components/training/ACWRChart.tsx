...
export const ACWRChart = ({ data }: ACWRChartProps) => {
  const hasReliableData = data.some((point) => point.acwr !== null);

  return (
    <Card className="p-4">
      <h3 className="text-sm font-semibold mb-1">Acute:Chronic Workload Ratio</h3>
      <p className="text-xs text-muted-foreground mb-3">
        7-day acute ÷ 28-day chronic rolling average · Green zone (0.8–1.3) = optimal
      </p>
      {!hasReliableData ? (
        <div className="h-[220px] flex items-center justify-center text-sm text-muted-foreground">
          Not enough data — ACWR appears after 4 weeks of logged sessions.
        </div>
      ) : (
